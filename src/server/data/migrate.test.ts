...
describe("migration runner", () => {
  ...
  it("applies 0001_init and a second run is a no-op (one row per version)", () => {
    t = makeTestDb(); // makeTestDb already ran migrate once

    // schema_migrations exists and has exactly one row (version 1).
    const rows = t.db
      .prepare("SELECT version FROM schema_migrations ORDER BY version")
      .all() as Array<{ version: number }>;
    expect(rows).toEqual([{ version: 1 }]);
    expect(getSchemaVersion(t.db)).toBe(1);
