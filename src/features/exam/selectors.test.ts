 ...
 *   - `countAnswered` counts entries with a selection, commit, or gaveUp.
...
describe("answerStatus", () => {
  it("returns 'current' when isCurrent is true, regardless of state", () => {
    // Even with selection, flag, and commit all set, the current question
    // is always reported as 'current' so the navigator can highlight it.
    expect(
      answerStatus(
        makeAnswer({ selected: ["A"], flagged: true, committed: true, gaveUp: true }),
        true,
      ),
    ).toBe("current");
  });
  ...
  it("returns 'answered_correct' when committed with a correct selection", () => {
    // Reveal + matching correctAnswer -> green.
    const q = makeQuestion({ correctAnswer: "A" });
    expect(
      answerStatus(makeAnswer({ committed: true, selected: ["A"] }), false, q),
  ...
  it("returns 'answered_correct' for a multi-select match", () => {
    ...
    expect(
      answerStatus(
        makeAnswer({ committed: true, selected: ["B", "A"] }),
        false,
        q,
      ),
    ).toBe("answered_correct");
  ...
  it("returns 'answered_incorrect' when committed with a wrong selection", () => {
    // Reveal + non-matching correctAnswer -> red.
    const q = makeQuestion({ correctAnswer: "A" });
    expect(
      answerStatus(makeAnswer({ committed: true, selected: ["B"] }), false, q),
    ).toBe("answered_incorrect");
  });

  it("returns 'answered_pending' when committed but correctAnswer is unknown", () => {
    // Reveal happened, but we don't have the correct key yet (e.g. live
    // question not refreshed with snapshot).
    expect(
      answerStatus(
        makeAnswer({ committed: true, selected: ["A"] }),
        false,
        makeQuestion(),
      ),
  ...
  it("returns 'gave_up' when committed with an empty selection", () => {
    // Reveal with no selection means the user gave up post-commit.
    expect(
      answerStatus(makeAnswer({ committed: true }), false),
    ).toBe("gave_up");
  });

  it("returns 'flagged' when flagged is true, committed is false, gaveUp is false, and no selection", () => {
  ...
  it("returns 'answered_pending' when selected is non-empty and not flagged/committed/current/gaveUp", () => {
    expect(answerStatus(makeAnswer({ selected: ["A"] }), false)).toBe(
      "answered_pending",
    );
  });

  it("returns 'answered_pending' for a multi-selection that hasn't been committed", () => {
...
describe("liveOutcome", () => {
  ...
  it("returns 'pending' when there is a selection but no commit", () => {
    expect(liveOutcome(makeAnswer({ selected: ["A"] }))).toBe("pending");
  });

  it("returns 'gave_up' when committed with no selection", () => {
    expect(liveOutcome(makeAnswer({ committed: true }))).toBe("gave_up");
  });

  it("returns 'pending' when committed with a selection but no correctAnswer on the question", () => {
    expect(
      liveOutcome(
        makeAnswer({ committed: true, selected: ["A"] }),
        makeQuestion(),
  ...
  it("returns 'correct' when the selection matches a single correctAnswer", () => {
    const q = makeQuestion({ correctAnswer: "A" });
    expect(
      liveOutcome(makeAnswer({ committed: true, selected: ["A"] }), q),
    ).toBe("correct");
  });
  ...
  it("returns 'incorrect' when the selection does not match a single correctAnswer", () => {
    const q = makeQuestion({ correctAnswer: "A" });
    expect(
      liveOutcome(makeAnswer({ committed: true, selected: ["B"] }), q),
    ).toBe("incorrect");
  });
...
describe("countAnswered", () => {
  ...
  it("counts entries that have a selection", () => {
    // Three selected questions, no commits → all three are touched.
...
describe("countFlagged", () => {
  ...
  it("counts only entries with flagged === true", () => {
    // Selection/commit/empty entries must NOT be counted as flagged.
    const answers: Record<number, AnswerState> = {
      1: makeAnswer({ selected: ["A"] }), // pending, not flagged
      2: makeAnswer({ committed: true }), // committed, not flagged
      3: makeAnswer(), // untouched
      4: makeAnswer({ flagged: true }), // ← counted
      5: makeAnswer({ flagged: true, selected: ["A"] }), // ← counted (flag + answer)
      6: makeAnswer({ flagged: true, committed: true }), // ← counted (flag + commit)
    };
...
describe("countGaveUp", () => {
  ...
  it("counts only entries with gaveUp === true", () => {
    const answers: Record<number, AnswerState> = {
      ...
      2: makeAnswer({ committed: true, selected: ["A"] }), // committed, not gave up
      3: makeAnswer(), // untouched
      4: makeAnswer({ gaveUp: true }), // ← counted
      5: makeAnswer({ gaveUp: true, selected: ["A"] }), // ← counted (gave up + select)
      6: makeAnswer({ gaveUp: true, committed: true }), // ← counted (gave up + commit)
    };
