...
- [ ] **F4-T16** (M) `<QuestionPanel>` + `<OptionList>`/`<OptionItem>`: single-choice radios; chip labels always render in the FIXED A, B, C, D order (ADR-15); the underlying option key the click handler receives is looked up from `optionOrder[i]`; post-reveal/lock shows per-option correctness styling.
