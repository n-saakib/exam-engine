...
## Plan Requirements
- Short summary: score %, correct / incorrect / **gave up** / revealed / unanswered, time taken.
- Detailed view: every question with your answer, correct answer, all explanations, tips.
- Filter detailed view: incorrect only, **gave up only**, revealed only, flagged only, or all.
...
- The detailed list shows, per question: your answer, the correct answer, all per-option explanations, and Tips, with correct/incorrect/gave-up/revealed styling. A question with `outcome="gave_up"` shows a "Gave up" pill on the card. **ADR-15:** the review surface mirrors the live exam view — options + explanations render in the same shuffled order the user saw during the exam, with chips in FIXED A, B, C, D order (the display letter). `correctAnswer` and `yourAnswer` (stored as underlying keys) are reverse-mapped to the display letter the user clicked, so the summary reads "Correct answer: A" and matches the chip the user actually pressed.
...
### Backend
- [ ] **F5-T1** (M) `GET /api/sessions/:id/results` → results DTO ([`03-api-specification.md` §5.1](../03-api-specification.md)) with answers + explanations + per-question `outcome` (one of `correct | incorrect | gave_up | revealed | unanswered`).
- [ ] **F5-T2** (S) `PATCH /api/sessions/:id/review` → set `is_bookmarked` / `note`; works on completed (and in-progress) sessions.
- [ ] **F5-T3** (M) `POST /api/sessions/:id/retake { scope, options }`: `all` → fresh session for the set; `incorrect` → new snapshot containing only origin questions whose outcome was `incorrect` / `revealed` / **`gave_up`** (post-ADR-14); set `origin_session_id`; `409` if `incorrect` scope but none qualify.
...
- [ ] **F5-T6** (M) `<ScoreSummaryCard>`: %, **five-way breakdown** (correct / incorrect / gave up / revealed / unanswered), time taken vs limit, difficulty/domain header.
- [ ] **F5-T7** (M) `<QuestionReviewList>` + `<QuestionReviewCard>`: your vs correct answer, all explanations, Tips, outcome styling (one of `correct | incorrect | gave_up | revealed | unanswered`); a gave-up card shows a "Gave up" pill on the outcome badge.
- [ ] **F5-T8** (M) `<DetailFilterBar>`: All / Incorrect only / **Gave up only** / Revealed only / Flagged — client-side filter over the DTO. The "Gave up" tab is its own filter between Incorrect and Revealed.
...
### §5 enhancement
- [ ] **F5-T11** (S) Wire "Retake incorrect only" prominently as the primary study-loop CTA; disable when no incorrect/revealed/**gave-up** exist.

---

## Testing
- [ ] Integration: results DTO matches the graded session; outcomes correct for a seeded session with one incorrect + one **gave-up** + one revealed + one unanswered.
- [ ] Integration: retake `all` creates a full new session; retake `incorrect` creates a session whose snapshot = the incorrect+revealed+**gave-up** subset; `409` when none.
