...
- **CHECK constraints** on every enum column (`status`, `mode`, `confidence`, `difficulty`, catalog `status`/`source`) — a `'in-progress'` typo must be rejected, not silently stored.
